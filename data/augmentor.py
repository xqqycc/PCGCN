...
class GraphAugmentor(object):
    ...
    @staticmethod
    def edge_increase(sp_adj, drop_rate):
        """Input: a sparse user-item adjacency matrix and a dropout rate."""
        adj_shape = sp_adj.get_shape()
        edge_count = sp_adj.count_nonzero()
        row_idx, col_idx = sp_adj.nonzero()
        increase_u_idx = np.random.choice(a=np.arange(adj_shape[0]),size=int(edge_count *drop_rate),replace=True)
        increase_i_idx = np.random.choice(a=np.arange(adj_shape[1]),size=int(edge_count *drop_rate),replace=True)
        user_np = np.append(row_idx,increase_u_idx)
        item_np = np.append(col_idx,increase_i_idx)
        edges = np.ones_like(user_np, dtype=np.float32)
        increased_adj = sp.csr_matrix((edges, (user_np, item_np)), shape=adj_shape)
        return increased_adj
