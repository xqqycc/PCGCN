...
class Interaction(Data,Graph):
    def __init__(self, conf, training_set_IF, test_set,training_set_IU,test_IF,test_IU):
        Graph.__init__(self)
        Data.__init__(self,conf,training_set_IF, test_set,training_set_IU,test_IF,test_IU)
        self.user = {}
        self.item = {}
        self.id2user = {}
        self.id2item = {}
        ...
        self.implicit_set = defaultdict(dict)
        self.test_set = defaultdict(dict)
        self.test_high_set = defaultdict(dict)
        self.test_low_set = defaultdict(dict)
        self.test_set = defaultdict(dict)
        self.test_set_item = set()
        self.__generate_set()
        self.user_num = len(self.user)
        self.item_num = len(self.item)
        self.total_num=self.user_num+self.item_num
        self.ui_adj = self.__create_sparse_bipartite_adjacency('train')
        self.ui_adj_implicit_self_connect=self.__create_sparse_bipartite_adjacency('implicit',self_connection=True)
        self.ui_adj_implicit=self.__create_sparse_bipartite_adjacency('implicit')
        self.norm_adj = self.normalize_graph_mat(self.ui_adj)
        self.norm_adj_implicit = self.normalize_graph_mat(self.ui_adj_implicit_self_connect)
        self.norm_adj_total=self.normalize_graph_mat(self.ui_adj_implicit+self.ui_adj)
        self.interaction_mat = self.__create_sparse_interaction_matrix('train')
        self.interaction_mat_implicit = self.__create_sparse_interaction_matrix('implicit')
    ...
    def __generate_set(self):
        ...
        for entry in self.implicit_data:
            user, item, rating = entry
            if item not in self.item:
                self.item[item] = len(self.item)
                self.id2item[self.item[item]] = item
        ...
        for entry in self.test_data:
            ...
            self.test_set_item.add(item)#已存在的item则不添加，未存在的则添加

        for entry in self.test_high_data:
            user, item, rating = entry
            self.test_high_set[user][item] = rating

        for entry in self.test_low_data:
            user, item, rating = entry
            self.test_low_set[user][item] = rating

    def __create_sparse_bipartite_adjacency(self,target, self_connection=False):
        '''
        ...
        if target=='train':
            ...
            n_nodes = self.user_num + self.item_num
            row_idx = [self.user[pair[0]] for pair in self.implicit_data]
            col_idx = [self.item[pair[1]] for pair in self.implicit_data]
            user_np = np.array(row_idx)
            item_np = np.array(col_idx)
            ratings = np.ones_like(user_np, dtype=np.float32)
    ...
    def __create_sparse_interaction_matrix(self,target):
        """
        ...
        if target=='train':
            ...
            row, col, entries = [], [], []
            for pair in self.implicit_data:
                row += [self.user[pair[0]]]
                col += [self.item[pair[1]]]
                entries += [1.0]
            interaction_mat = sp.csr_matrix((entries, (row, col)), shape=(self.user_num, self.item_num),
                                            dtype=np.float32)
            return interaction_mat
    ...
    def training_size(self):
        return self.user_num, self.item_num, len(self.training_data_total)
